...
import { getBody, assertBody, passTime, assertDomUpdates, fakedom } from "./helpers";
...
test('destroy element is removed as soon as its transitions finish', async () => {
  let finish!: () => void;
  fakedom.mockAnimations(() => [{finished: new Promise<void>((res) => { finish = res; })}]);
  let data = A.proxy(true);
  A(() => {
    if (data.value) A('b', {destroy: "x"});
  });
...
test('destroy class starting no transition removes the element right away', async () => {
...
test('a cancelled destroy transition also releases its element', async () => {
  let cancel!: (reason: Error) => void;
  fakedom.mockAnimations(() => [{finished: new Promise((_res, rej) => { cancel = rej; })}]);
  let data = A.proxy(true);
  A(() => {
    if (data.value) A('b', {destroy: "x"});
  });
...
test('a destroy animation that never finishes hits the 3-second backstop', async () => {
  // The fakedom default is a never-finishing animation.
  let data = A.proxy(true);
  A(() => {
    if (data.value) A('b', {destroy: "x"});
  });
