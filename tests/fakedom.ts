...
// Bun lacks `Temporal`, so install a mock. Each type uses a private brand so that
// calling a method on a proxied instance throws, matching the real behavior.
const Temporal: Record<string, new (...args: any[]) => object> = {};
for (const name of "Duration Instant PlainDate PlainDateTime PlainMonthDay PlainTime PlainYearMonth ZonedDateTime".split(" ")) {
    Temporal[name] = class {
        #brand = true;
        toString() {
            this.#brand; // throws if `this` is a proxy rather than the real instance
...
const globals = {Node, Element, TextNode, document, window, setTimeout, queueMicrotask, getComputedStyle, location, history, URLSearchParams, Temporal};
